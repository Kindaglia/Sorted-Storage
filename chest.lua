...
local formspec_sorted_chest = "size[10,12.5]"..  -- Increased width to accommodate the mini-inventory
    ...
    "list[current_name;mini;9.25,0.8;1,1;]"..  -- Mini-inventory slot moved to the right
    mcl_formspec.get_itemslot_bg(9.25,0.8,1,1)..  -- Add background for the mini-inventory slot
...
minetest.register_node("sortedstorage:sorted_chest", {
    ...
    on_construct = function(pos)
        ...
        -- Define the mini-inventory (1 slot)
        inv:set_size("mini", 1)
    ...
    can_dig = function(pos, player)
        local meta = minetest.get_meta(pos)
        local inv = meta:get_inventory()
        return inv:is_empty("main") and inv:is_empty("mini") -- Allow digging only if both inventories are empty
    end,
})
