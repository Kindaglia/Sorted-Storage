...
local formspec_big_magic_chest = "size[18,14]"..  -- Formspec size adjusted for larger inventory
    "label[0,0;"..minetest.formspec_escape(minetest.colorize("#313131", "Big Magic Chest")).."]"..
    "image_button[17.1,0;0.75,0.75;"..sorting_icon..";sort_inventory;]"..  -- Button with sorting icon (larger and placed higher)
    "list[current_name;main;0,0.8;18,6;]"..  -- Inventory list (18 columns, 6 rows)
    mcl_formspec.get_itemslot_bg(0,0.8,18,6)..  -- Add background for the node inventory slots
    "label[4,7.0;"..minetest.formspec_escape(minetest.colorize("#313131", "Inventory")).."]"..
    "list[current_player;main;4,7.5;9,3;9]"..
    mcl_formspec.get_itemslot_bg(4,7.5,9,3)..  -- Add background for the player's inventory slots
    "list[current_player;main;4,10.74;9,1;]"..
    mcl_formspec.get_itemslot_bg(4,10.74,9,1)..  -- Add background for the final row of the player's inventory
    "listring[current_name;main]"..  -- Link the node inventory
    "listring[current_player;main]"  -- Link the player's inventory

minetest.register_node("sortedstorage:big_magic_chest", {
    description = "Big Magic Chest",
    tiles = {
        "big_magic_chest_up.png",          -- Top
        "big_magic_chest_side.png",  -- Bottom
        "big_magic_chest_side.png",  -- Right
        "big_magic_chest_side.png",  -- Left
        "big_magic_chest_side.png",  -- Back
        "big_magic_chest_front.png",  -- Front
    },
    drawtype = "nodebox",
    paramtype = "light",
    paramtype2 = "facedir",
    ...
    on_construct = function(pos)
        ...
        -- Define the inventory (18 columns, 6 rows)
        inv:set_size("main", 18 * 6)  -- 18 columns, 6 rows for the Big Magic Chest

        -- Set the formspec
        meta:set_string("formspec", formspec_big_magic_chest)
        
        -- Set the informational text for the node
        meta:set_string("infotext", "Big Magic Chest")
    end,
    ...
    on_rightclick = function(pos, node, player, itemstack, pointed_thing)
        ...
        minetest.sound_play("open_big_magic_chest", {
            pos = pos,
        ...
        minetest.show_formspec(player:get_player_name(), "sortedstorage:big_magic_chest_"..minetest.pos_to_string(pos), meta:get_string("formspec"))
    end,
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    if formname:match("^sortedstorage:big_magic_chest_") then
        if fields.sort_inventory then
            local player_name = player:get_player_name()
            local pos = minetest.string_to_pos(formname:match("big_magic_chest_(.+)"))
            if not pos then
